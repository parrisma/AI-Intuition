...
if [[ -z "${KAFKA_EXTERNAL}" ]]; then
  ...
  sed "s/EXTERNAL:\/\/123.456.789.123.:99999/EXTERNAL:\/\/${KAFKA_EXTERNAL}/g" ./config/server.properties > ./config/new_sp
  mv ./config/new_sp ./config/server.properties
fi
...
if [[ -z "${KAFKA_PLAINTEXT}" ]]; then
  ...
  sed "s/PLAINTEXT:\/\/localhost:9092/PLAINTEXT:\/\/${KAFKA_PLAINTEXT}/g" ./config/server.properties > ./config/new_sp
  mv ./config/new_sp ./config/server.properties
fi
